...
function showGameScoreLayer() {
    ...
    score_text += "</span>" + ' 次哦！';
    score_text += "<br>相当于 <span style='color:red;'>" + (_gameScore * 15000 / deviation_time).toFixed(2) + "</span> BPM 下的十六分音符哦！(蔡徐坤弄得)"
...
function shareText(score) {
    ...
    if (score <= 5 * __Time) return '还没到蔡徐坤的程度!';
    if (score <= 7.5 * __Time) return '马上就要超过蔡徐坤的程度!';
    if (score <= 10 * __Time) return '你应该已经练了两年半了吧!';
    return '蔡徐坤:又有一位IKUN加入我们的荔枝集团了!';
}
